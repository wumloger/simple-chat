	...
	<view class="container">
		...
		<view class="cu-list menu-avatar" style="border-top: none;">
			<view class="cu-item" :class="modalName=='move-box-'+ index?'move-cur':''" @touchstart="ListTouchStart"
				...
				<view class="cu-avatar round lg" :style="[{backgroundImage:'url('+cell.groupAvatar+')'}]">
				</view>
				<view class="content">
					<view class="text-grey">{{cell.groupName}}</view>
...
<script>
	...
	export default {
		...
		methods: {
			getForOthers() {
				request("/group/getMyInvitations").then((res) => {
					console.log(res);
					this.list = res.data
				})
			},
